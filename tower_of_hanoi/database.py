...
def init_db():
    ...
    c.execute('CREATE DATABASE IF NOT EXISTS puzzle_game_hub')
    c.execute('USE puzzle_game_hub')
    
    # Create tables
    c.execute('''
    CREATE TABLE IF NOT EXISTS tower_of_hanoi_user_games (
    ...
    c.execute('''
    CREATE TABLE IF NOT EXISTS tower_of_hanoi_algorithm_performance (
...
def save_user_game(player_name, disk_count, moves_count, move_sequence):
    ...
    c = conn.cursor()
    c.execute('''
    INSERT INTO tower_of_hanoi_user_games (player_name, disk_count, moves_count, move_sequence)
    VALUES (%s, %s, %s, %s)
...
def save_algorithm_performance(algorithm, disk_count, execution_time, moves_count, parameters=None, notes=None):
    ...
    c = conn.cursor()
    c.execute('''
    INSERT INTO tower_of_hanoi_algorithm_performance (algorithm, disk_count, execution_time, moves_count, parameters, notes)
    VALUES (%s, %s, %s, %s, %s, %s)
...
def get_algorithm_benchmarks():
    ...
    FROM tower_of_hanoi_algorithm_performance 
