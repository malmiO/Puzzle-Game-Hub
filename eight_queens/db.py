...
def init_db():
    ...
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS eight_queens_player_answers (
        id INT AUTO_INCREMENT PRIMARY KEY,
...
def save_player_answer(player_name, correct_solution):
    ...
    cursor.execute('''
    INSERT INTO eight_queens_player_answers (player_name, correct_solution, created_at)
    VALUES (%s, %s, %s)
    ''', 
...
def solution_exists(method):
    ...
    cursor.execute("SELECT COUNT(*) FROM eight_queens_solutions WHERE method = %s", (method,))
...
def save_to_db(method, time_taken, solutions):
    ...
    # Check for duplicate
    cursor.execute("SELECT solutions FROM eight_queens_solutions WHERE method = %s", (method,))
    ...
    cursor.execute('''
    INSERT INTO eight_queens_solutions (method, time_taken, total_solutions, solutions, created_at)
    VALUES (%s, %s, %s, %s, %s)
    ''', 
...
def player_solution_exists(player_name, solution):
    ...
    cursor.execute('''
    SELECT COUNT(*) FROM eight_queens_player_answers WHERE player_name = %s AND correct_solution = %s
    ''', 
...
def get_recognized_solutions():
    ...
    cursor.execute("SELECT eight_queens_correct_solution FROM player_answers WHERE correct_solution IS NOT NULL")
...
def is_solution_recognized(solution):
    ...
    cursor.execute("SELECT COUNT(*) FROM eight_queens_player_answers WHERE correct_solution = %s", (json.dumps(solution),))
    exists = cursor.fetchone()[0] > 0
...
def count_unique_player_solutions():
    ...
    cursor.execute("SELECT DISTINCT correct_solution FROM eight_queens_player_answers WHERE correct_solution IS NOT NULL")
...
def reset_player_solutions():
    ...
    cursor.execute("DELETE FROM eight_queens_player_answers")
    conn.commit()
    conn.close()