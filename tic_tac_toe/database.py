...
def initialize_database():
    ...
    try:
        ...
        with temp_engine.connect() as conn:
            conn.execute(text("CREATE DATABASE IF NOT EXISTS tictactoe"))
            conn.execute(text("USE tictactoe"))
        ...
        with engine.connect() as conn:
            ...
            if game_id is None:
                ...
                result_obj = conn.execute(
                    text("INSERT INTO games (player_name, result, moves) VALUES (:player_name, :result, :moves)"),
                ...
                conn.execute(
                    text("UPDATE games SET player_name = :player_name, result = :result, moves = :moves WHERE id = :game_id"),
                    {"player_name": player_name, "result": result, "moves": moves_str, "game_id": game_id}
                )
...
def save_algorithm_performance(game_id: int, move_number: int, greedy_time: float, minimax_time: float, update_game_id: bool = False):
    if engine is None:
        st.error("Database engine not initialized.")
        return
    ...
    try:
        with engine.connect() as conn:
            if update_game_id:
                ...
                conn.execute(
                    text("UPDATE algorithm_performance SET game_id = :game_id WHERE game_id IS NULL"),
                    {"game_id": game_id}
                )
            else:
                # Insert new performance record
                conn.execute(
                    text("INSERT INTO algorithm_performance (game_id, move_number, greedy_time, minimax_time) VALUES (:game_id, :move_number, :greedy_time, :minimax_time)"),
                    {
                        "game_id": game_id,
...
def get_performance_data(limit=10):
    ...
    try:
        query = """
            ...
            FROM games g
            JOIN algorithm_performance ap ON g.id = ap.game_id
            GROUP BY g.id
        ...
        df_melted['algorithm'] = df_melted['algorithm'].replace({
            'total_greedy_time': 'Greedy',
            'total_minimax_time': 'Minimax'
        })
        return df_melted
    except OperationalError as e:
